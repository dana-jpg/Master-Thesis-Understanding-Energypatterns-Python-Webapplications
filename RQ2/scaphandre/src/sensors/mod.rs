        ...
        if let Some(record) = self.get_proc_tracker().get_process_last_record(pid) {
            ...
            if let Some(conso) = &topo_conso {
                ...
                res.insert(
                    ...
                    (
                        String::from("Power consumption attributed to the process, in microwatts"),
                        Record::new(record.timestamp, result.to_string(), units::Unit::MicroWatt),
                    ),
                );
            }

            // PKG power consumption (sum of all socket PKG power)
            let mut total_pkg_power: f64 = 0.0;
            for socket in &self.sockets {
                if let Some(socket_power) = socket.get_records_diff_power_microwatts() {
                    total_pkg_power += socket_power.value.parse::<f64>().unwrap_or(0.0);
                }
            }
            if total_pkg_power > 0.0 {
                let result = (total_pkg_power * process_cpu_percentage as f64) / 100.0_f64;
                res.insert(
                    String::from("scaph_process_power_consumption_pkg_microwatts"),
                    (
                        String::from("Power consumption attributed to the process from CPU package, in microwatts"),
                        Record::new(record.timestamp, result.to_string(), units::Unit::MicroWatt),
                    ),
                );
            }

            // DRAM power consumption (sum of all DRAM domain power)
            let mut total_dram_power: f64 = 0.0;
            for socket in &self.sockets {
                for domain in &socket.domains {
                    if domain.name == "dram" {
                        if let Some(dram_power) = domain.get_records_diff_power_microwatts() {
                            total_dram_power += dram_power.value.parse::<f64>().unwrap_or(0.0);
                        }
                    }
                }
            }
            if total_dram_power > 0.0 {
                let result = (total_dram_power * process_cpu_percentage as f64) / 100.0_f64;
                res.insert(
                    String::from("scaph_process_power_consumption_dram_microwatts"),
                    (
                        String::from("Power consumption attributed to the process from DRAM, in microwatts"),
                        Record::new(record.timestamp, result.to_string(), units::Unit::MicroWatt),
                    ),
                );
            }
