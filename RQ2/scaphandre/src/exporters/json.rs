...
impl Drop for JsonExporter {
    fn drop(&mut self) {
        if !self.is_finished {
            let _ = self.out_writer.write("]".as_bytes());
            let _ = self.out_writer.flush();
        }
    }
}
...
struct Consumer {
    ...
    consumption: f32,
    // Custom additions for per-process PKG and DRAM power
    consumption_pkg: Option<f32>,
    consumption_dram: Option<f32>,
    timestamp: f64,
...
impl Exporter for JsonExporter {
    ...
    fn run(&mut self) {
        ...
        if let Some(timeout) = self.time_limit {
            ...
            // Handle both SIGINT (Ctrl-C) and SIGTERM (terminate())
        ...
        // Explicitly close JSON array and flush before exiting
        let _ = self.out_writer.write("]".as_bytes());
        let _ = self.out_writer.flush();
...
impl JsonExporter {
    ...
    fn retrieve_metrics(&mut self) {
        ...
        let metrics_iter = metrics.iter();
        let socket_metrics: Vec<_> = metrics.iter().filter(|x| x.name == "scaph_socket_power_microwatts").collect();
        // Fixed: now collects all socket metrics for multi-socket systems
                ...
                metrics
                    ...
                    .find(|x| {
                        x.name == "scaph_process_power_consumption_microwatts"
                            && x.attributes.get("pid").unwrap() == &pid_str
                    })
                    .map(|metric| {
                        // Find PKG metric for this process
                        let consumption_pkg = metrics
                            .iter()
                            .find(|x| {
                                x.name == "scaph_process_power_consumption_pkg_microwatts"
                                    && x.attributes.get("pid").unwrap() == &pid_str
                            })
                            .map(|m| format!("{}", m.metric_value).parse::<f32>().unwrap_or(0.0));

                        // Find DRAM metric for this process
                        let consumption_dram = metrics
                            .iter()
                            .find(|x| {
                                x.name == "scaph_process_power_consumption_dram_microwatts"
                                    && x.attributes.get("pid").unwrap() == &pid_str
                            })
                            .map(|m| format!("{}", m.metric_value).parse::<f32>().unwrap_or(0.0));
        ...
        let all_sockets = self
            ...
            .iter()
            .filter_map(|socket| {
                if let Some(metric) = socket_metrics.iter().find(|x| {
                    socket.id
                        == x.attributes
                            .get("socket_id")
                            .unwrap()
