#!/usr/bin/env python3
"""
Item Defaults - Energy Measurement

Measures energy consumption of get_item_defaults function.
"""
...
RESULTS_FILE = os.environ.get("ENERGY_RESULTS_FILE", "energy_results_item_defaults.json")
...
def run_single_trial(trial_idx: int, baseline: Optional[BaselineMeasurement] = None) -> TrialResult:
    ...
    if pid == 0:
        ...
        for _ in tqdm(range(MEASURE_RUNS), desc=f"Trial {trial_idx}", disable=bool(os.environ.get("CI")), mininterval=2.0, file=sys.stderr):
            result = get_item_defaults(ITEM, COMPANY)
            _ = frappe.as_json(result)
...
if __name__ == "__main__":
    ...
    results = ExperimentResults(
        config={
            "experiment_label": "get_item_defaults",
            "site": SITE,
            "item": ITEM,
            "company": COMPANY,
            "measure_runs": MEASURE_RUNS,
