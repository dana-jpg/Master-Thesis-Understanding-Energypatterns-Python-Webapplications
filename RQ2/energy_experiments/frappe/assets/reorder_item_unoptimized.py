...
def get_items_for_reorder() -> dict[str, list]:
	"""
	UNOPTIMIZED version: Uses old 2-step approach.
	Step 1: Get item names via SQL
	Step 2: Fetch reorder data per-item in separate queries
	"""
	# Step 1: Get item names (old SQL approach)
	item_names = frappe.db.sql_list(
		"""
		select name
		from `tabItem` item
		where is_stock_item = 1
		  ...
		  and (
				end_of_life is null
				or end_of_life = '0000-00-00'
				or end_of_life > %(today)s
		  )
		  and (
				exists (
					select name
				...
				or (
					variant_of is not null
					and variant_of != ''
					and exists (
						select name
						from `tabItem Reorder` ir
						where ir.parent = item.variant_of
					)
				)
		  )
		""",
	...
	# Step 2: For each item, fetch reorder data separately (unoptimized N+1 query pattern)
	...
	for item_code in item_names:
		...
		item_data = frappe.db.get_value(
			"Item",
			item_code,
			["name", "stock_uom", "purchase_uom", "description", "item_name", 
			 "item_group", "brand", "variant_of", "has_variants", "lead_time_days"],
			as_dict=True,
		)
		
		# Get reorder levels for this item
		reorder_data = frappe.db.get_all(
			"Item Reorder",
			filters={"parent": item_code},
			fields=["warehouse", "warehouse_group", "material_request_type", 
					"warehouse_reorder_level", "warehouse_reorder_qty"],
		)
		
		# If no direct reorder data, check variant_of
		if not reorder_data and item_data.get("variant_of"):
			reorder_data = frappe.db.get_all(
				"Item Reorder",
				filters={"parent": item_data.variant_of},
				fields=["warehouse", "warehouse_group", "material_request_type",
						"warehouse_reorder_level", "warehouse_reorder_qty"],
			)
		
		for rd in reorder_data:
			combined = frappe._dict({**item_data, **rd})
			itemwise_reorder.setdefault(item_code, []).append(combined)
