...
class AccountsController(TransactionBase):
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		...
		for row in ref_wise_billed_amount.values():
			# ❌ NON-OPTIMIZED: DB + role lookup INSIDE loop
			role_allowed_to_overbill = frappe.get_single_value(
				"Accounts Settings", "role_allowed_to_over_bill"
			)
			is_overbilling_allowed = role_allowed_to_overbill in frappe.get_roles()

			total_billed_amt = row.billed_amt
			allowance = get_allowance_for(
				row.item_code, {}, None, None, "amount"
			)[0]

			max_allowed_amt = flt(row.ref_amt * (100 + allowance) / 100)

			if total_billed_amt < 0 and max_allowed_amt < 0:
				# while making debit note against purchase return entry
				total_billed_amt, max_allowed_amt = (
					abs(total_billed_amt),
					abs(max_allowed_amt),
				)
		...
		if total_overbilled_amt > 0.1:
			# role check must be re-done to match old semantics
			role_allowed_to_overbill = frappe.get_single_value(
				"Accounts Settings", "role_allowed_to_over_bill"
			)
			if role_allowed_to_overbill in frappe.get_roles():
				frappe.msgprint(
	...
	def validate_multiple_billing_opt(self, ref_dt, item_ref_dn, based_on):
		from erpnext.controllers.status_updater import get_allowance_for
