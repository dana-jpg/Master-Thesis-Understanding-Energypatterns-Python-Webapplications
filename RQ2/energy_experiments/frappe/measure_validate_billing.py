#!/usr/bin/env python3
"""
Validate Multiple Billing - Energy Measurement

Measures energy consumption of validate_multiple_billing function.
Uses fork-based process isolation for accurate measurement.
"""
...
PROCESS_NAME = "erpnext_overbilling_worker"
BENCH_INVOICE = "ACC-SINV-2025-00011"
REF_DOCTYPE = "Sales Order"
ITEM_REF_FIELD = "so_detail"
BASED_ON = "rate"
...
RESULTS_FILE = os.environ.get("ENERGY_RESULTS_FILE", "energy_results_validate_billing.json")
...
def run_single_trial(trial_idx: int, baseline: Optional[BaselineMeasurement] = None) -> TrialResult:
    ...
    if pid == 0:
        ...
        invoice = frappe.get_doc("Sales Invoice", BENCH_INVOICE)

        for _ in tqdm(range(MEASURE_RUNS), desc=f"Trial {trial_idx}", disable=bool(os.environ.get("CI")), mininterval=2.0, file=sys.stderr):
            invoice.validate_multiple_billing(REF_DOCTYPE, ITEM_REF_FIELD, BASED_ON)
...
if __name__ == "__main__":
    ...
    results = ExperimentResults(
        config={
            "experiment_label": "Validate Multiple Billing",
            "site": SITE,
            "bench_invoice": BENCH_INVOICE,
            "ref_doctype": REF_DOCTYPE,
            "item_ref_field": ITEM_REF_FIELD,
            "based_on": BASED_ON,
            "measure_runs": MEASURE_RUNS,
