...
def strip_keyword_from_regex(qa):
    qa_without_regex_symbols = re.sub(r'\\b|.\?', '', qa)
    return qa_without_regex_symbols


def keyword_sorter(qa: str):
    qa_without_regex_symbols = strip_keyword_from_regex(qa)
    return -qa.count(" "), -qa.count("-"), -len(qa_without_regex_symbols), qa_without_regex_symbols


def transform_keywords(qas: PatternsMap, sorter=keyword_sorter, *, keep_regex_notation=True):
    return {qa: sorted(keywords if keep_regex_notation else (strip_keyword_from_regex(keyword) for keyword in keywords),
